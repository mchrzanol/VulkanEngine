...
void texturesLoading::createImage(VkDevice device, VkPhysicalDevice physicalDevice, uint32_t width, uint32_t height, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage& image, VkDeviceMemory& imageMemory) {
...
void texturesLoading::createTextureImage(VkDevice device, VkPhysicalDevice physicalDevice, VkCommandPool commandPool, VkQueue graphicsQueue, std::string name, std::string path) {
...
void texturesLoading::transitionImageLayout(VkDevice device,VkCommandPool commandPool,VkQueue graphicsQueue, VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout) {
    VkCommandBuffer commandBuffer = vkinit::beginSingleTimeCommands(device, commandPool);
    ...
    vkinit::endSingleTimeCommands(device, commandBuffer, commandPool, graphicsQueue);
}

void texturesLoading::copyBufferToImage(VkDevice device, VkCommandPool commandPool, VkQueue graphicsQueue, VkBuffer buffer, VkImage image, uint32_t width, uint32_t height) {
    VkCommandBuffer commandBuffer = vkinit::beginSingleTimeCommands(device, commandPool);
...
void texturesLoading::createTextureImageView(VkDevice device, std::string name) {
...
void texturesLoading::cleanup(VkDevice device) {
    ...
    for (auto& texture : textures) {
        vkDestroyImageView(device, texture.second.textureImageView, nullptr);
